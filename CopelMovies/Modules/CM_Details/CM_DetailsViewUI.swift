...
protocol CM_DetailsViewUIDelegate {
    func notifyBackTapped()
    func notifyMenuPressed()
    func notifyUpdateFavorite(isFavorite: Bool, movieId: Int)
}

class CM_DetailsViewUI: UIView{
    var delegate: CM_DetailsViewUIDelegate?
    ...
    public lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(CM_GenresCollectionViewCell.self, forCellWithReuseIdentifier: CM_GenresCollectionViewCell.identifier)
    ...
    lazy var companyCollection: UICollectionView = {
        ...
        let collectionView = UICollectionView(frame: .zero, collectionViewLayout: layout)
        collectionView.register(CM_CompanysCollectionViewCell.self, forCellWithReuseIdentifier: CM_CompanysCollectionViewCell.identifier)
        collectionView.translatesAutoresizingMaskIntoConstraints = false
        collectionView.delegate = self
        collectionView.dataSource = self
        collectionView.backgroundColor =  #colorLiteral(red: 0.03991495073, green: 0.08235343546, blue: 0.1102337912, alpha: 1)
    ...
    public convenience init(
        ...
        delegate: CM_DetailsViewUIDelegate,
        movieId: Int,
    ...
    public func updateValues(movieDetails: MovieDetails, isFavorite: Bool) {
        ...
        if let url = URL(string: CM_NetworkManager.shared.firstURL + (movieDetails.posterPath ?? "")) {
...
extension CM_DetailsViewUI: CM_NavigationBarDelegate {
    func backTapped() {
...
extension CM_DetailsViewUI: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView.tag == 0 {
            if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CM_GenresCollectionViewCell.identifier, for: indexPath) as? CM_GenresCollectionViewCell {
                cell.genresTitleLabel.text = genresData[indexPath.row]
                return cell
            }
        }else if collectionView.tag == 1 {
            if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CM_CompanysCollectionViewCell.identifier, for: indexPath) as? CM_CompanysCollectionViewCell {
                cell.companysLabel.text = companysData[indexPath.row].name
                if let url = URL(string: CM_NetworkManager.shared.firstURL + (companysData[indexPath.row].logoPath ?? "") ) {
                    let session = URLSession.shared
                    let task = session.dataTask(with: url) { (data, response, error) in
                        if let data = data, let image = UIImage(data: data) {
                            DispatchQueue.main.async {
